...
The demo uses a static LINZ reference chart screenshot, fictional coordinates and simulated movement at 8 knots. It does not request GPS, notification, wake-lock or persistent-storage permission. Spoken proximity alerts are optional and activate only after the visitor selects **Sound on**. Saved Places filters control both the sample map and list, while simulated proximity-alert logic remains active for hidden categories. Anchor Watch is demonstrated independently.
